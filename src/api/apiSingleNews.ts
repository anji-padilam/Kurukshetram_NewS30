...
export const getSingleNews = async (newsId: string): Promise<SingleNewsResponse['result']> => {
  ...
  console.log(`[getSingleNews] Current language ID: ${getCurrentLanguageId()}`);
  ...
  if (!singleNewsSuccess) {
      ...
      const comprehensiveCategoryIds = [
        ...
        '1e2f3a4b-5c6d-7e8f-9a0b-1c2d3e4f5a6b'  // Overseas News
          ...
          console.log('Found news item via Politics-style search:', newsItem);
            ...
            language_id: (newsItem as any).language_id || (newsItem as any).languageId || getCurrentLanguageId(),
    ...
    try {
      ...
      for (const languageId of languageIds) {
        try {
          const languageResponse = await apiClient.get<ApiResponse<NewsResponse>>('/news/filter-multi-categories', {
            params: {
    ...
    try {
      ...
      for (const params of searchParams) {
        try {
          ...
          const paramResponse = await apiClient.get<ApiResponse<NewsResponse>>('/news/filter-multi-categories', {
            params: {
              language_id: getCurrentLanguageId(),
              limit: params.limit,
              page: params.page
    ...
    try {
      ...
      const finalResponse = await apiClient.get<ApiResponse<NewsResponse>>('/news/filter-multi-categories', {
        params: {
          language_id: getCurrentLanguageId(),
          limit: 10000,
          page: 1
    ...
    return getFallbackNewsData(newsId);
  }
};

// Fallback data function
const getFallbackNewsData = (newsId: string): any => {
  ...
  return {
    ...
    excerpt: "Stay informed with the latest news and updates from our team.",
    shortNewsContent: "Stay informed with the latest news and updates from our team.",
    longNewsContent: {
